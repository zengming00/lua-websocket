...
local function writeWebSocketMessageHeader(c, header)
    ...
    local data = tabToStr {
        bor(opcode, rsv, fin), bor(payloadLength, mask)
    }
    assert(c:send(data))
end
...
local function handler(c)
    ...
    while line ~= "" do
        -- print('"' .. line .. '"')
        if key == nil then
            key = string.match(line, "^Sec%-WebSocket%-Key:%s*(.*)")
        end
        line = assert(c:receive("*l"))
    end

    key = sha.sha1(key .. "258EAFA5-E914-47DA-95CA-C5AB0DC85B11")
    key = sha.bin2base64(sha.hex2bin(key))
    ...
    c:send(table.concat(handshakeMsg))
